---
title: "Investigating the sequencing_summary file of barcode 7 (Genomic tip 100/G) from SUPER accuracy rebasecalling barcodes trimmed, )"
...
```{r}
sequencing_summary <- read_delim ("C:/Users/keegk/OneDrive - moredungroup/Karen Keegan/Nanopore/SQKRBK004/Extraction_experiment_run_22.2.23/Rebasecalling/Rebasecalled_trimmed_super_accuracy/barcode_7/sequencing_summary_barcode7.txt")

extexp2025_barcode7_remove_reads <- sequencing_summary %>%
...
extexp2025_barcode7_remove_reads <- save(extexp2025_barcode7_remove_reads, file = "extexp2025_barcode7_remove_reads")
load("extexp2025_barcode7_remove_reads") #this was saved with 2025 written after it but the actual file name loads in without the 2025 bit after it just FYI.
...
#  geom_text(aes(50000, 2000, label=paste("Total number of reads = ", No_reads))) +
 ## geom_text(aes(50000, 1900, label=paste("Total number of bases = ", No_bases))) +
 # geom_text(aes(50000, 1800, label=paste("Mean read length = ", Mean_read))) +
 # geom_text(aes(50000, 1700, label=paste("Max read length = ", Max_read))) +
 # geom_text(aes(50000, 1600, label=paste("N50 = ", Median))) +
#  geom_text(aes(50000, 1500, label=paste("Mean Q score = ", Mean_q_score))) +
 # geom_text(aes(50000, 1400, label=paste("Percentage of reads that passed  = ", Percentage))) +
